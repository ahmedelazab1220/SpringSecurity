...
public class AuthenticationController {
	...
	public ResponseEntity<?> logout(HttpServletRequest request, HttpServletResponse response) {
		...
		return ResponseEntity.ok(Map.of("message", "Successfully logged out.", "status", HttpServletResponse.SC_OK,
				"timestamp", System.currentTimeMillis(), "path", request.getRequestURI()));
	}

	@PostMapping("/refreshToken")
	public JwtResponse refreshToken(@RequestBody RefreshTokenRequest refreshTokenRequest) {
		RefreshToken refresh_token = refreshTokenService.findByToken(refreshTokenRequest.getRefresh_token());
