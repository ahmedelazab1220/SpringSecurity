	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(csrf -> csrf.disable())
				.authorizeHttpRequests(request -> request.requestMatchers("/api/v2/auth", "/api/v1/auth/**").permitAll()
						...
						.requestMatchers("/api/v2/auth/admin/write").hasAuthority("ADMIN_WRITE").anyRequest()
						.authenticated())
				.httpBasic(httpBasic -> httpBasic.authenticationEntryPoint(this.authEntry))
				.exceptionHandling(ex -> ex.accessDeniedHandler(this.accessEntry)).formLogin(form -> form.permitAll());
